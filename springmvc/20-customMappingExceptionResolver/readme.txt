20-customMappingExceptionResolver:自定义的异常处理器。
自定义的异常处理器，是要实现HandleExceptionResovler接口，编写异常的处理代理。

自定义异常处理器的步骤：
1.定义类实现HandleExceptionResovler接口
2.在springmvc配置中声明自定义的异常处理器。
